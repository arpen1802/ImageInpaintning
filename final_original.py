# -*- coding: utf-8 -*-
"""Final Original.ipynb

Automatically generated by Colaboratory.

Original file is located at
    https://colab.research.google.com/drive/1H-z2nurM_NGxCxz_rJeW0ZpYcWz40tBk
"""
...
class Encoder(nn.Module):
    ...
    def __init__(self, cnum=32):
        ...
        self.conv1 = nn.Conv2d(6, cnum, [5, 5], stride=[1, 1], padding=0)
        self.conv2 = nn.Conv2d(cnum, cnum*2, [3, 3], stride=[2, 2], padding=0)
        self.conv3 = nn.Conv2d(cnum*2, cnum*2, [3, 3], stride=[1, 1], padding=0)
        self.conv4 = nn.Conv2d(cnum*2, cnum*4, [3, 3], stride=[2, 2], padding=0)
        self.conv5 = nn.Conv2d(cnum*4, cnum*4, [3, 3], stride=[1, 1], padding=0)
        self.conv6 = nn.Conv2d(cnum*4, cnum*8, [3, 3], stride=[2, 2], padding=0)
        self.dil_conv1 = nn.Conv2d(cnum*8, cnum*8, [3, 3], stride=[1, 1], padding=0, dilation=2)
        self.dil_conv2 = nn.Conv2d(cnum*8, cnum*8, [3, 3], stride=[1, 1], padding=0, dilation=4)
        self.dil_conv3 = nn.Conv2d(cnum*8, cnum*8, [3, 3], stride=[1, 1], padding=0, dilation=8)
        self.dil_conv4 = nn.Conv2d(cnum*8, cnum*8, [3, 3], stride=[1, 1], padding=0, dilation=16)
        self.act = nn.ELU()
    ...
    def forward(self, x):
        ...
        x = self.pad2(x)
        x = self.act(self.dil_conv1(x))
        x = self.pad4(x)
        x = self.act(self.dil_conv2(x))
        x = self.pad8(x)
        x = self.act(self.dil_conv3(x))
        x = self.pad16(x)
        x = self.act(self.dil_conv4(x))
        return x
...
class Decoder(nn.Module):
    ...
    def __init__(self, cnum=256, size1=256, size2=256):
        ...
        self.pad1 = nn.ReflectionPad2d((1, 1, 1, 1))
        # Model convolutions
        self.conv1 = nn.Conv2d(cnum, cnum//2, [3, 3], stride=[1, 1], padding=0)
        self.conv2 = nn.Conv2d(cnum//2, cnum//2, [3, 3], stride=[1, 1], padding=0)
        self.conv3 = nn.Conv2d(cnum//2, cnum//4, [3, 3], stride=[1, 1], padding=0)
        self.conv4 = nn.Conv2d(cnum//4, cnum//4, [3, 3], stride=[1, 1], padding=0)
        self.conv5 = nn.Conv2d(cnum//4, cnum//8, [3, 3], stride=[1, 1], padding=0)
        self.conv6 = nn.Conv2d(cnum//8, cnum//8, [3, 3], stride=[1, 1], padding=0)
        self.conv7 = nn.Conv2d(cnum//8, cnum//16, [3, 3], stride=[1, 1], padding=0)
        self.conv8 = nn.Conv2d(cnum//16, cnum//16, [3, 3], stride=[1, 1], padding=0)
        self.conv9 = nn.Conv2d(cnum//16, 3, [3, 3], stride=[1, 1], padding=1)
        # Model activation
        self.act = nn.ELU()
...
class Discriminator_red(nn.Module):
    ...
    def __init__(self, cnum = 64):
        ...
        # Model convolutions
        self.conv1 = spectral_norm(nn.Conv2d(3, cnum, [5, 5], stride=[2, 2], padding=2))
        self.conv2 = spectral_norm(nn.Conv2d(cnum, cnum*2, [5, 5], stride=[2, 2], padding=2))
        self.conv3 = spectral_norm(nn.Conv2d(cnum*2, cnum*4, [5, 5], stride=[2, 2], padding=2))
        self.conv4 = spectral_norm(nn.Conv2d(cnum*4,  cnum*4, [5, 5], stride=[2, 2], padding=2))
        self.conv5 = spectral_norm(nn.Conv2d(cnum*4,  cnum*4, [5, 5], stride=[2, 2], padding=2))
        self.conv6 = spectral_norm(nn.Conv2d(cnum*4,  cnum*8, [5, 5], stride=[2, 2], padding=2))
...
# hyperparaters
d_lr = 0.0008 # discriminator learning rate
g_lr = 0.0008 # generator learning rate
batch_size = 8
train_epochs = 100
valid_epoch = 20
